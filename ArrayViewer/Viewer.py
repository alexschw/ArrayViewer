...
class ViewerWindow(QMainWindow):
    ...
    def __initMenu(self):
        ...
        _menu_opt(menuStart, "Delete All Data", self._delete_all_data, "Ctrl+X")
        ...
        self.combine_opt = _menu_opt(self.contextMenu, "Combine Dataset", self._dlg_combine)
    ...
    def _dropdown(self, _):
        ...
        if self.datatree.current_item():
            self.combine_opt.setVisible(self.datatree.current_item().childCount() != 0)
    ...
    def _dlg_load_data(self):
        ...
        if FD.exec_():
            ...
            self.config.set("opt", "first_to_last", str(checkbox.checkState() != 0))
            # For all files
    ...
    def _dlg_reshape(self):
        ...
        if cTree.currentItem().childCount() != 0:
            ...
            if self.datatree.is_files_tree():
                keys = [
                    tr + [k] for k in self.datatree.similar_items
                    if isinstance(self.get(tr + [k]), (np.ndarray, h5py.Dataset))
                ]
            else:
                keys = [
                    [k] + tr for k in self.datatree.similar_items
                    if isinstance(self.get([k] + tr), (np.ndarray, h5py.Dataset))
                ]
            if len(keys) == 0:
                return
    ...
    def _load_slice(self):
        ...
        k = self._slice_key()
        return (
            self.slices[k] if k in self.slices else None,
    ...
    def transpose_data(self, new_order):
        ...
        if self._slice_key() in self.slices:
            self.slices[self._slice_key()] = [self.slices[self._slice_key()][i] for i in new_order]
    ...
    def _save_chart(self):
        ...
        if figure:
            fname = QFileDialog.getSaveFileName(self, "Save Image", "./figure.png", ftype)
            if fname:
    ...
    def load_files(self, fnames):
        ...
        for fname in fnames:
            ...
            if key in self.files or key in new_keys:
                ...
                replaceBtn = QPushButton(QIcon.fromTheme("list-add"), "Add new Dataset")
